...
def clean_transactions(input_path, file_id, target_table, db_url, db_user, db_password):
    spark = SparkSession.builder.appName("CleanTransactions").getOrCreate()
    df = spark.read.option("header", "true").csv(input_path)
    df = df.withColumn("_file_id", lit(int(file_id)))

    mandatory = ["transaction_date", "from_account", "to_account", "amount", "description"]
    ...
    if "transaction_date" in df.columns:
        valid_condition = valid_condition & to_date(col("transaction_date"), "yyyy-MM-dd").isNotNull()
    ...
    if valid_count > 0:
        ...
        df_valid = df_valid.withColumn("transaction_date", to_date(col("transaction_date"), "yyyy-MM-dd"))
        for field in ["from_account", "to_account", "description"]:
        ...
        cols_to_save = ["transaction_date", "from_account", "to_account", "amount", "description", "currency", "file_id", "version"]
    ...
    if invalid_count > 0:
        df_errors = df_invalid.withColumn("row_json", to_json(struct([col(c) for c in df_invalid.columns]))) \
            .select(
                ...
                lit("Missing mandatory fields or invalid date/amount/currency").alias("error_reason"),
...
if __name__ == "__main__":
    if len(sys.argv) < 7:
        print("Usage: clean_transactions.py <input_path> <file_id> <target_table> <db_url> <db_user> <db_password>")
        sys.exit(1)
    ...
    clean_transactions(input_path, file_id, target_table, db_url, db_user, db_password)